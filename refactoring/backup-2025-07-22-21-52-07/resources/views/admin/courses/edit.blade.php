<x-layouts.admin title="Editar Curso">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('admin.courses.show', $course) }}" class="bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white p-3 rounded-xl transition-all duration-300 hover:scale-105">
                        ...
                        <div>
                            ...
                            <p class="text-purple-100 mt-1">{{ $course->title }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required
                                    ...
                                    <option value="mandatory" {{ old('type', $course->type) === 'mandatory' ? 'selected' : '' }}>Obrigatório</option>
                                    <option value="optional" {{ old('type', $course->type) === 'optional' ? 'selected' : '' }}>Opcional</option>
                                    <option value="certification" {{ old('type', $course->type) === 'certification' ? 'selected' : '' }}>Certificação</option>
                                </select>
                                @error('type')
                                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="order_index" id="order_index" value="{{ old('order_index', $course->order_index) }}" min="0"
                                       class="w-full bg-slate-700/50 border border-slate-600 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 placeholder-slate-400"
                                       placeholder="0">
                                @error('order_index')
                                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="6" required
                                      class="w-full bg-slate-700/50 border border-slate-600 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 placeholder-slate-400"
                                      placeholder="Descreva detalhadamente o conteúdo e objetivos do curso...">{{ old('description', $course->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" id="tags" value="{{ old('tags', $course->tags ? implode(', ', $course->tags) : '') }}"
                                   class="w-full bg-slate-700/50 border border-slate-600 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 placeholder-slate-400"
                ...
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-slate-700/50 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        @if($course->thumbnail)
                            <div>
                                <label class="block text-sm font-semibold text-white mb-2">Imagem Atual</label>
                                <div class="flex items-center space-x-4">
                                    <img src="{{ $course->thumbnail_url }}" alt="{{ $course->title }}" class="w-24 h-24 object-cover rounded-lg">
                                    <div>
                                        <p class="text-slate-300 text-sm">Imagem atual do curso</p>
                                        <p class="text-slate-400 text-xs">Selecione uma nova imagem abaixo para substituir</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <div>
                            <label for="thumbnail" class="block text-sm font-semibold text-white mb-2">
                                {{ $course->thumbnail ? 'Nova Imagem de Capa' : 'Imagem de Capa' }}
                            </label>
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-slate-400">
                                        <label for="thumbnail" class="relative cursor-pointer bg-slate-800 rounded-md font-medium text-purple-400 hover:text-purple-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-purple-500">
                                            <span>{{ $course->thumbnail ? 'Alterar imagem' : 'Enviar uma imagem' }}</span>

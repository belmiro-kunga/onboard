<x-layouts.admin :title="'Adicionar Módulo ao Curso: ' . $course->title">
    <div class="px-4 sm:px-6 lg:px-8">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white mb-6">Novo Módulo para {{ $course->title }}</h1>
        <form action="{{ route('admin.courses.modules.store', $course) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="order_index" id="order_index" value="{{ $nextOrderIndex ?? 1 }}" min="1" required class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                </div>
                <div>
            ...
            <div>
                ...
                <a href="{{ route('admin.courses.modules.index', $course) }}" class="ml-4 text-gray-600 dark:text-gray-300 hover:underline">Cancelar</a>
            </div>
        </form>
    </div>
</x-layouts.admin> 